# LANTERN_WOWSQL_READ_QUERIES_V3

Status: CANONICAL WOWSQL READ CONTRACT / FREE-SHARED COMPATIBILITY ROUTE
...
## Preflight â€” projection binding
...
B0 and B1 must match exactly. On mismatch, retry the complete preflight -> B0 -> payload -> B1 sequence once. A second mismatch yields `UNKNOWN`.
...
A stable projected read proves current governed material for the exact frozen WoWSQL projection only. It does not grant producer authority, material-admission authority, database write authority, Project-settings authority, merge/deploy authority, or runtime identity continuity.
