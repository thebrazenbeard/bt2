...
Status: `COMPATIBILITY_INTERFACE_ADAPTER`. Normative topology is `native/project/BT2_PERSISTENT_INTERFACE_TOPOLOGY_V1.json`. No merge/cutover implied.
...
This compatibility surface must not be treated as a second independent topology truth. It is valid only while it agrees exactly with the normative persistent-interface contract and the 13-role reconstruction map.
