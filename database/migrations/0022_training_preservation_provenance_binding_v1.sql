...
CREATE OR REPLACE FUNCTION bt2.training_preservation_provenance_digest_v1(
  ...
  p_source_set_digest_sha256 text
) RETURNS text
LANGUAGE sql
IMMUTABLE
AS $function$
    ...
    public.digest(
      convert_to(
        jsonb_build_array(
          ...
          p_manifest_digest_sha256,p_source_set_digest_sha256
        )::text,
        'UTF8'
      ),
      'sha256'
...
AS $function$
  SELECT bt2.training_preservation_provenance_digest_v1(
    p_agent_key,p_version,p_source_repository,p_source_ref,p_source_commit,p_source_tree,
    p_source_package_path,p_target_manifest_path,p_package_tree_git_sha1,p_manifest_blob_git_sha1,
    NULL,NULL
  )
$function$;
...
INSERT INTO bt2.training_preservation_provenance_bindings_v1(
  ...
  manifest_digest_sha256,source_set_digest_sha256,provenance_digest_sha256
)
SELECT
  ...
  d.manifest_digest_sha256,d.source_set_digest_sha256,
...
CROSS JOIN LATERAL (
  SELECT
    CASE v.preservation_migration_key
      WHEN 'BT2-TRAINING-FIVE-V1.0.0-BYTE-PRESERVATION-V1' THEN '164ee92a70f6585dece59dca09ab84bd6020a4d950393b127091aed91713687c'
      WHEN 'BT2-TRAINING-SIX-V1.0.0-BYTE-PRESERVATION-V1' THEN '32ed842c8bb6c2ccc49bb327189da9b03178b403f05ad6b10cca9b5f1ceeff38'
      WHEN 'BT2-TRAINING-NINE-V1.0.0-BYTE-PRESERVATION-V1' THEN 'a0702c71cd14521cf88312ee9f07c6bf5ccb5a95079b047332e33a532f32e67b'
      WHEN 'BT2-TRAINING-THIRTEEN-V1.0.0-BYTE-PRESERVATION-V1' THEN '49ca83e55940caecf6df56170bcd5ddca8044933d56db897fbfe0f052c87fc3e'
      ELSE NULL
    END::text AS manifest_digest_sha256,
    CASE v.preservation_migration_key
      WHEN 'BT2-TRAINING-FOUR-V1.0.1-BYTE-PRESERVATION-V1' THEN '1070866d342043c21d06d9bc384fbf7cf78d231850ef2edef514b3e95229c332'
      WHEN 'BT2-TRAINING-FIVE-V1.0.0-BYTE-PRESERVATION-V1' THEN '80d2aab944ee8c8e13b83ae99aacc4ba530bc42cda69ed6c9ab21958a64e55f8'
      WHEN 'BT2-TRAINING-SIX-V1.0.0-BYTE-PRESERVATION-V1' THEN 'ef44176582819750193c7d591e9ee449ea8c3d6743a8bff3eb5228baf4fad1cc'
      WHEN 'BT2-TRAINING-NINE-V1.0.0-BYTE-PRESERVATION-V1' THEN 'bcbee02e5e7f59b763ba1a441a66c4e3b83210befcb85ab7f66a5d0ccf7769e7'
      WHEN 'BT2-TRAINING-THIRTEEN-V1.0.0-BYTE-PRESERVATION-V1' THEN '89f3da76b9033c9302e2ddb85c68b6bf0b7856383d7bce65f671170d4a8a7bc1'
      ELSE NULL
    END::text AS source_set_digest_sha256
) d;
...
BEGIN
  IF NEW.source_binding_state = 'BYTE_PRESERVED_VERIFIED' THEN
    ...
    ELSIF NEW.manifest_digest_sha256 IS DISTINCT FROM v_binding.manifest_digest_sha256 THEN
      RAISE EXCEPTION 'PRESERVATION_PROVENANCE_MISMATCH:manifest_digest_sha256';
    ELSIF NEW.source_set_digest_sha256 IS DISTINCT FROM v_binding.source_set_digest_sha256 THEN
    ...
    IF v_receipt.evidence ? 'manifest_digest_sha256'
       AND v_receipt.evidence->>'manifest_digest_sha256' IS DISTINCT FROM v_binding.manifest_digest_sha256 THEN
      RAISE EXCEPTION 'PRESERVATION_RECEIPT_PROVENANCE_CONTRADICTION:manifest_digest_sha256';
    END IF;
    IF v_receipt.evidence ? 'source_set_digest_sha256'
       AND v_receipt.evidence->>'source_set_digest_sha256' IS DISTINCT FROM v_binding.source_set_digest_sha256 THEN
      RAISE EXCEPTION 'PRESERVATION_RECEIPT_PROVENANCE_CONTRADICTION:source_set_digest_sha256';
    END IF;
  END IF;
...
BEGIN
  ...
  LEFT JOIN bt2.training_preservation_provenance_bindings_v1 b
    ON b.preservation_migration_key=mr.migration_key
  WHERE tp.source_binding_state='BYTE_PRESERVED_VERIFIED'
    AND (
      b.preservation_migration_key IS NULL
      OR mr.result_state <> 'VERIFIED'
      OR lower(COALESCE(mr.evidence->>'target_package_tree',mr.evidence->>'source_package_tree'))
           IS DISTINCT FROM lower(b.package_tree_git_sha1)
      ...
      OR tp.agent_key IS DISTINCT FROM b.agent_key
      OR tp.version IS DISTINCT FROM b.version
      OR tp.source_repository IS DISTINCT FROM b.source_repository
      OR tp.source_ref IS DISTINCT FROM b.source_ref
      OR tp.source_commit IS DISTINCT FROM b.source_commit
      OR tp.source_tree IS DISTINCT FROM b.source_tree
      OR tp.source_package_path IS DISTINCT FROM b.source_package_path
      OR tp.manifest_path IS DISTINCT FROM b.target_manifest_path
      OR lower(tp.package_tree_git_sha1) IS DISTINCT FROM lower(b.package_tree_git_sha1)
      OR lower(tp.manifest_blob_git_sha1) IS DISTINCT FROM lower(b.manifest_blob_git_sha1)
      OR tp.manifest_digest_sha256 IS DISTINCT FROM b.manifest_digest_sha256
      OR tp.source_set_digest_sha256 IS DISTINCT FROM b.source_set_digest_sha256
    )
  LIMIT 1;
...
FOR EACH ROW EXECUTE FUNCTION bt2.guard_training_package_source_binding_v1();
  ...
  CONSTRAINT training_preservation_binding_manifest_digest_sha256_check
    CHECK (manifest_digest_sha256 IS NULL OR manifest_digest_sha256 ~ '^[0-9a-f]{64}
    CHECK (provenance_digest_sha256 ~ '^[0-9a-f]{64}$'),
  CONSTRAINT training_preservation_binding_package_identity_uniq
    UNIQUE (agent_key,version,package_tree_git_sha1)
);
...
FOR EACH ROW EXECUTE FUNCTION bt2.guard_training_package_source_binding_v1();
  ...
  CONSTRAINT training_preservation_binding_source_set_digest_sha256_check
    CHECK (source_set_digest_sha256 IS NULL OR source_set_digest_sha256 ~ '^[0-9a-f]{64}
    CHECK (provenance_digest_sha256 ~ '^[0-9a-f]{64}$'),
  CONSTRAINT training_preservation_binding_package_identity_uniq
    UNIQUE (agent_key,version,package_tree_git_sha1)
);
