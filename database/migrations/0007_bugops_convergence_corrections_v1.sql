-- Corrections discovered during source-first review of 0006 before live application.
-- Final migration chain semantics are 0006 + 0007.

-- Closure evidence provenance includes the recording actor in the idempotency digest.
...
-- Intake semantic identity is a digest of the normalized report request, not of the
-- operation envelope (which includes the operation UUID/idempotency receipt subject).
...
DECLARE
  ...
  v_actor text; v_request jsonb; v_intake_digest text; v_op_event uuid; v_attempt uuid; v_verified uuid;
  v_op record; v_bug record; v_existing_event record;
  v_bug_id uuid:=gen_random_uuid(); v_event uuid:=gen_random_uuid();
  v_queue uuid; v_q_replay boolean; v_snapshot jsonb; v_snapshot_digest text; v_result jsonb;
BEGIN
  ...
  v_request:=jsonb_build_object(
    ...
    'severity',v_severity,'component',p_component,'reported_by',p_reported_by,
  ...
  IF FOUND THEN
    ...
    v_result:=jsonb_build_object('bug_id',v_bug.bug_id,'event_id',v_existing_event.bug_event_id,'state_version',1,
    ...
    v_verified:=bt2.append_operation_event_v1(p_operation_id,v_attempt,'VERIFIED',NULL,'no_retry_required',
    ...
    RETURN;
  END IF;
  INSERT INTO bt2.bugs(bug_id,intake_key,intake_digest_sha256,title,description,severity,status,component,reported_by,
    assigned_agent_key,state_version,routing_revision,dispatch_revision,evidence)
  VALUES(v_bug_id,v_key,v_intake_digest,v_title,coalesce(p_description,''),v_severity,'NEW',p_component,p_reported_by,
    v_target,1,1,1,p_evidence);
  v_snapshot:=jsonb_build_object('bug_id',v_bug_id,'event_id',v_event,'state_version',1,'routing_revision',1,
    'dispatch_revision',1,'target_agent_key',v_target,'action','BUG_REPORTED','operation_id',p_operation_id);
  ...
  INSERT INTO bt2.bug_events(bug_event_id,bug_id,state_version,routing_revision,dispatch_revision,event_type,actor_agent_key,
  ...
  VALUES(v_event,v_bug_id,1,1,1,'REPORTED',v_actor,p_operation_id,v_queue,v_op.request_digest_sha256,v_request,
    jsonb_build_object('assigned_agent_key',v_target),v_snapshot,v_snapshot_digest);
  v_result:=jsonb_build_object('bug_id',v_bug_id,'event_id',v_event,'state_version',1,'routing_revision',1,
    'dispatch_revision',1,'queue_item_id',v_queue,'adopted_existing',false);
  v_verified:=bt2.append_operation_event_v1(p_operation_id,v_attempt,'VERIFIED',NULL,'no_retry_required',
    'Bug report committed',v_result,
...
-- Route actors are durable active BT2 agents; raw external reporter text remains supported only at intake.
...
) RETURNS TABLE(out_event_id uuid,out_state_version bigint,out_routing_revision bigint,out_dispatch_revision bigint,
  out_queue_item_id uuid,idempotent_replay boolean)
LANGUAGE plpgsql AS $function$
DECLARE v_target text:=lower(btrim(p_target_agent_key)); v_actor text:=lower(btrim(p_actor_agent_key));
  v_request jsonb; v_root uuid; v_attempt uuid; v_done uuid; v_op record; v_bug record;
  v_event uuid:=gen_random_uuid(); v_queue uuid; v_q_replay boolean; v_state bigint; v_route bigint; v_dispatch bigint;
...
BEGIN
  ...
  v_request:=jsonb_build_object('kind','ROUTE_BUG','bug_id',p_bug_id,'expected_state_version',p_expected_state_version,
    'expected_dispatch_revision',p_expected_dispatch_revision,'target_agent_key',v_target,'actor_agent_key',v_actor,'reason',p_reason);
  v_root:=bt2.prepare_operation_v1(p_operation_id,NULL,v_actor,NULL,'ROUTE_BUG','bt2.bugs:'||p_bug_id::text,p_operation_id::text,v_request);
  SELECT * INTO v_op FROM bt2.operations WHERE operation_id=p_operation_id;
  IF v_op.state='VERIFIED' THEN
    RETURN QUERY SELECT (v_op.result_payload->>'event_id')::uuid,(v_op.result_payload->>'state_version')::bigint,
      (v_op.result_payload->>'routing_revision')::bigint,(v_op.result_payload->>'dispatch_revision')::bigint,
      (v_op.result_payload->>'queue_item_id')::uuid,true; RETURN;
  ELSIF v_op.state<>'PREPARED' THEN RAISE EXCEPTION 'OPERATION_REQUIRES_RECONCILIATION state=%',v_op.state; END IF;
  ...
  v_snapshot:=jsonb_build_object('bug_id',p_bug_id,'event_id',v_event,'state_version',v_state,'routing_revision',v_route,
  ...
  INSERT INTO bt2.bug_events(bug_event_id,bug_id,state_version,routing_revision,dispatch_revision,event_type,actor_agent_key,
    operation_id,queue_item_id,request_digest_sha256,request_payload,details,dispatch_snapshot,dispatch_payload_digest_sha256)
  VALUES(v_event,p_bug_id,v_state,v_route,v_dispatch,'ROUTED',v_actor,p_operation_id,v_queue,v_op.request_digest_sha256,v_request,
    jsonb_build_object('from_agent_key',v_bug.assigned_agent_key,'to_agent_key',v_target,'reason',p_reason),v_snapshot,v_snapshot_digest);
  v_result:=jsonb_build_object('event_id',v_event,'state_version',v_state,'routing_revision',v_route,'dispatch_revision',v_dispatch,'queue_item_id',v_queue);
...
) RETURNS TABLE(out_event_id uuid,out_state_version bigint,out_status text,out_routing_revision bigint,
  out_dispatch_revision bigint,out_queue_item_id uuid,idempotent_replay boolean)
LANGUAGE plpgsql AS $function$
DECLARE v_requested text:=upper(btrim(p_status)); v_actor text:=lower(btrim(p_actor_agent_key));
...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM bt2.agents WHERE agent_key=v_actor AND active) THEN RAISE EXCEPTION 'INVALID_ACTOR_AGENT'; END IF;
  v_request:=jsonb_build_object('kind','UPDATE_BUG_STATUS','bug_id',p_bug_id,'expected_state_version',p_expected_state_version,
    'requested_status',v_requested,'actor_agent_key',v_actor,'note',p_note);
  v_root:=bt2.prepare_operation_v1(p_operation_id,NULL,v_actor,NULL,'UPDATE_BUG_STATUS','bt2.bugs:'||p_bug_id::text,p_operation_id::text,v_request);
  SELECT * INTO v_op FROM bt2.operations WHERE operation_id=p_operation_id;
  IF v_op.state='VERIFIED' THEN
    RETURN QUERY SELECT (v_op.result_payload->>'event_id')::uuid,(v_op.result_payload->>'state_version')::bigint,
      v_op.result_payload->>'status',(v_op.result_payload->>'routing_revision')::bigint,
      (v_op.result_payload->>'dispatch_revision')::bigint,nullif(v_op.result_payload->>'queue_item_id','')::uuid,true; RETURN;
  ELSIF v_op.state<>'PREPARED' THEN RAISE EXCEPTION 'OPERATION_REQUIRES_RECONCILIATION state=%',v_op.state; END IF;
  SELECT * INTO v_bug FROM bt2.bugs WHERE bug_id=p_bug_id FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'BUG_NOT_FOUND'; END IF;
  IF v_bug.state_version<>p_expected_state_version THEN RAISE EXCEPTION 'STALE_STATE_VERSION'; END IF;
  ...
  IF v_requested='REOPENED' THEN
    ...
    v_snapshot:=jsonb_build_object('bug_id',p_bug_id,'event_id',v_event,'state_version',v_state,'routing_revision',v_route,
      'dispatch_revision',v_dispatch,'target_agent_key',v_target,'action','BUG_REOPENED','operation_id',p_operation_id);
    v_snapshot_digest:=encode(digest(convert_to(v_snapshot::text,'UTF8'),'sha256'),'hex');
    SELECT out_queue_item_id,idempotent_replay INTO STRICT v_queue,v_q_replay
    ...
    UPDATE bt2.bugs SET status='NEW',assigned_agent_key=v_target,state_version=v_state,routing_revision=v_route,
      dispatch_revision=v_dispatch,updated_at=clock_timestamp(),closed_at=NULL WHERE bug_id=p_bug_id;
  ELSE
    ...
    UPDATE bt2.bugs SET status=v_status,state_version=v_state,dispatch_revision=v_dispatch,
      updated_at=clock_timestamp(),closed_at=CASE WHEN v_status='CLOSED' THEN clock_timestamp() ELSE closed_at END WHERE bug_id=p_bug_id;
  END IF;
  INSERT INTO bt2.bug_events(bug_event_id,bug_id,state_version,routing_revision,dispatch_revision,event_type,actor_agent_key,
    operation_id,queue_item_id,request_digest_sha256,request_payload,details,dispatch_snapshot,dispatch_payload_digest_sha256)
  VALUES(v_event,p_bug_id,v_state,v_route,v_dispatch,CASE WHEN v_requested='REOPENED' THEN 'REOPENED' ELSE 'STATUS_CHANGED' END,
