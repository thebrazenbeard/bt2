...
-- Synthetic-scope only. The transaction rolls back on success; no real PROJECT_LANTERN permit/profile/material/receipt is created or changed.
...
BEGIN
  ...
  VALUES(v_schema,repeat('c',64),repeat('d',64),ARRAY['semantic_role','subject_key']::text[]);
  ...
  INSERT INTO bt2.material_producer_permits(
    ...
    v_profile,v_policy,clock_timestamp()-interval '1 minute',clock_timestamp()+interval '10 minutes',NULL
  );
