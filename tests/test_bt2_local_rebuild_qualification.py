...
def write_package(tmp_path):
    ...
    entries = manifest['ordered_apply'] + manifest['post_preservation_data_loads'] + manifest['qualification']['smoke_tests']
    for rel in [x['path'] for x in entries]:
        path = tmp_path / rel
...
def test_manifest_sequence_excludes_post_preservation_load_but_verifies_it(tmp_path):
    mod = load_module()
